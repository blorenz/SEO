...
/**
* $Id: _form.php 219 2006-06-15 19:04:23Z kaneda $
*/


// added 11 May 2006 to solve idiocy issues
if (!isset($_REQUEST['vid'])) {
   $v_info = db_row(db_do("SELECT vehicle_id FROM auctions WHERE id = '$_GET[id]'"));
   $vid = $v_info['vehicle_id'];
}
...
<?php
	...
	if (!empty($errors) && $reserve_price_orig > $reserve_price && !empty($lowerReserveFlag)) //JJM 7/6/2010 added lowerReserveFlag, so this doesn't show up when it shouldn't
		echo "<div align='center'><input type='radio' name='confirm' value='yes'>Yes
			<input type='radio' name='confirm' value='no' checked>No<br><br></div>";
?>
...
<?php if (empty($bid_count) || ($bid_count == 0)) { ?>
    <tr>
     ...
     <input type="hidden" name="condition" value="<?php echo htmlentities($condition); ?>" />

<?php
   $p_data = mysql_fetch_assoc(mysql_query("SELECT photo_id FROM vehicles WHERE id = '$vid'"));

   if(empty($p_data['photo_id'])) {

      $new_photo = db_do("SELECT id FROM photos WHERE vehicle_id='$vid' ORDER BY id LIMIT 1");

      if(db_num_rows($new_photo) == 1) {
         list($photo) = db_row($new_photo);
      }
      ...
      $src = "../uploaded/$p_data[photo_id].jpg";
   }
?>
...
<?php } else {?>
	<tr>
     ...
     <td class="normal"><input type="hidden" name="bid_count" value="<?php echo $bid_count; ?>">
	 					<input type="hidden" name="description" value="<?php echo $description; ?>"><?php echo $description; ?></td>
    </tr>
	<tr>
     <td align="right" class="header" valign="top">Add to Description:</td>
     <td class="normal"><textarea name="add_description" rows="5" cols="65" wrap="virtual"><?php echo $add_description; ?></textarea></td>
    </tr>
	<tr>
     <td align="right" class="header" valign="top">Condition:</td>
     <td class="normal"><input type="hidden" name="condition" value="<?php echo $condition; ?>"><?php echo $condition; ?></td>
    </tr>
	<tr>
     <td align="right" class="header" valign="top">Add to Condition:</td>
     <td class="normal"><textarea name="add_condition" rows="5" cols="65" wrap="virtual"><?php echo $add_condition; ?></textarea></td>
    </tr>
<?php } ?>
...
?>
      ...
      <select name="ends_hour">
       <option value="0" <?php if ($ends_hour == '0') echo 'selected'; ?>>12:00 AM</option>
       <option value="1" <?php if ($ends_hour == '1') echo 'selected'; ?>>1:00 AM</option>
       <option value="2" <?php if ($ends_hour == '2') echo 'selected'; ?>>2:00 AM</option>
       <option value="3" <?php if ($ends_hour == '3') echo 'selected'; ?>>3:00 AM</option>
       <option value="4" <?php if ($ends_hour == '4') echo 'selected'; ?>>4:00 AM</option>
       <option value="5" <?php if ($ends_hour == '5') echo 'selected'; ?>>5:00 AM</option>
       <option value="6" <?php if ($ends_hour == '6') echo 'selected'; ?>>6:00 AM</option>
       <option value="7" <?php if ($ends_hour == '7') echo 'selected'; ?>>7:00 AM</option>
       <option value="8" <?php if ($ends_hour == '8') echo 'selected'; ?>>8:00 AM</option>
       <option value="9" <?php if ($ends_hour == '9') echo 'selected'; ?>>9:00 AM</option>
...
<?php  } else {?>
			<tr>
     <td align="right" class="header">Auction Starts:</td>
     <td class="normal"><?php echo $starts; ?>
	 <input type="hidden" name="starts" value="<?php echo $starts; ?>"><br></td>
    </tr>
    <tr>
     <td align="right" class="header">Auction Ends:</td>
     <td class="normal"><?php echo $ends; ?>
	 <input type="hidden" name="ends" value="<?php echo $ends; ?>"><br></td>
    </tr>
<?php  } ?>
	<tr><td colspan="2">&nbsp;</td></tr>
    <tr>
     <td align="right" class="header">Bid price starts at:</td>
     <td class="normal"><input type="text" name="minimum_bid" value="<?php echo $minimum_bid; ?>" size="35">
	 <input type="hidden" name="minimum_bid_orig" value="<?=$minimum_bid_orig?>"></td>
    </tr>
    <tr>
...
?>
	...
	<tr><td colspan="2">&nbsp;</td></tr>
    <tr>
     <td align="right" class="header">Reserve Price:<input type="hidden" name="current_bid" value="<?php echo $current_bid; ?>"></td>
    ...
    <tr>
     <td align="right" class="header">Verify NO RESERVE:</td>
     <td class="normal"><input type="checkbox" name="no_reserve" value="yes" />
	 <i>(check here to verify a NO RESERVE auction)</i></td>
    </tr>
	<tr><td colspan="2">&nbsp;</td></tr>
    <tr>
     <td align="right" class="header">Disable Bidding:</td>
     <td class="normal"><input type="checkbox" name="invoice" value="yes" <?php if ($invoice == 'yes') echo 'checked'; ?>>
	 <i>(Confirm that your auction is 'Buy Now' only)</i></td>
    </tr>
    <tr>
...
<!--	<tr>
     <td align="right" class="header">Custom Buy Now:</td>
     <td class="normal"><input type="text" name="buy_now_end" value="?php echo $buy_now_end; ?" size="35">
	 	<i>(optional - the price value that inactives the Buy Now feature)</i></td>
    </tr>  -->
    <tr>
	 ...
	 <?php if (empty($bid_count) || ($bid_count == 0)) { ?>
			<select name="pays_transport">
				...
				<option value="Other - (see item description)" <?php if ($pays_transport == 'Other - (see item description)') echo 'selected'; ?>>Other - (see item description)</option>
			</select>
	 <?php } else { ?>
			<?=$pays_transport?>
	 <?php } ?>
