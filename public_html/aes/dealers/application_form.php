...
$result = db_do("SELECT id FROM users WHERE username='$username'");
	...
	$result = db_do("SELECT d.address1, d.address2, d.bd_account, d.bd_name, d.bd_routing, d.billing_address1, d.billing_address2, 
			...
			d.poc_phone, d.poc_title, d.state, d.topdog_name, topdog_title, vst_ex, vst_no, years, d.zip, d.status, type, feed_provider, CONCAT(u.first_name, ' ', u.last_name) as ae_name  FROM dealers d, users u, aes ae  WHERE d.id=$id AND d.ae_id = ae.id AND u.id = ae.user_id"); 
