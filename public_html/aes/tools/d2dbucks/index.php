...
if(empty($filter))
{
    $sql = "SELECT COUNT(*) FROM d2dbucks WHERE ae_id='$ae_id'";
}
else
{
    $field = $$category;
    $sql = "SELECT COUNT(*) FROM d2dbucks, aes 
			WHERE d2dbucks.ae_id=aes.id AND $field LIKE \"%$search%\" and ae_id=$ae_id";
}
...
if(empty($filter))
{
	$result = db_do("SELECT d2dbucks.id, d2dbucks.serial_id, d2dbucks.amount, dms.id, CONCAT(dms.first_name, ' ', dms.last_name),
				aes.id, CONCAT(aes.first_name, ' ', aes.last_name), d2dbucks.dealer_id, d2dbucks.status, DATE_FORMAT(d2dbucks.modified, '%d-%b-%Y')
			FROM d2dbucks, dms, aes
			WHERE d2dbucks.dm_id=dms.id and d2dbucks.ae_id=aes.id and aes.id='$ae_id'
...
else
{
	...
	$result = db_do("SELECT d2dbucks.id, d2dbucks.serial_id, d2dbucks.amount, dms.id, CONCAT(dms.first_name, ' ', dms.last_name), d2dbucks.ae_id, 
			...
			FROM d2dbucks, dms, aes
			WHERE d2dbucks.dm_id=dms.id and d2dbucks.ae_id=aes.id and aes.id='$ae_id' AND $field LIKE \"%$search%\"
			ORDER BY $SortListBy $dir 
			LIMIT $_start, $limit");
}
...
<?php
	$bgcolor = '#FFFFFF';
	while (list($id, $serial_num, $amount, $dm_id, $dm_name, $ae_id, $ae_name, $dealer__id, $status, $modified) = db_row($result)) {
		...
		if ($dealer__id > 0) { 
			$result_dealer = db_do("SELECT name FROM dealers WHERE id='$dealer__id'");
			list($dealer_name) = db_row($result_dealer);
			$edit.= "&dealer__id=$dealer__id";
		}
		else
