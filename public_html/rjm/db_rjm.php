...
function echo_condition_report($id)
{
   $result = db_do("SELECT categories.name, auctions.title, auctions.status, " .
	...
	$exterior_color, $warranty, $title_item, $title_status, $certified, $veh_short_desc, $veh_long_desc, $rating,
      ...
      $wheels, $stereo_speakers, $suspension, $primary_drive, $instrument_panel, $bags_cond, $radiator, $windscreen, $series, $warebars, $bellypan, $tunnel, $skis, $track) = db_row($result);
...
<br> <br><br>
 <?php
                             // Why doesn't the return or next line work?????    \n.
 $test = array( "pink"=>"carnation",  "red"=>"rose");
 extract($test);
 echo "My favorite red flower is a $red.<br>\n";

 echo "My favorite pink flower is a $pink.<br>\n";

?>
<br><br><br>

<input type="text" name="phone" value="<?php echo $red ?>"><br>


// NEW SECTION BELOW  rjm  Listing 10-1:  A Script That Displays an HTML form. p 206 php for dummies.



<?php
/* Script name:   displayForm
*  Description:  Script displays a form and populates the form fields with the values of an array.
*/
	...
	echo "<p align='center'>
			<b>Please check the information below and change any information that is incorrect.</b>
		  <hr>
		  <form action='processform.php' method='POST'>
	      <table width='95%' border='0' celspacing='0'  cellpadding='2'>\n";
	foreach($customer as $field=>$value)
	{
		echo "<tr>
				<td align='right'> <B>  {$labels[$field]} </br></td>
				<td><input type='text' name='field' size='27'
					maxlength='25' value='{$customer[$field]}'></td>
			  </tr>";
	}
	echo "</table>
	...
	db_do ();
	db_connect();
	echo_condition_report(2525);

	echo "$username (randy)  jay         menges     <br>";
	echo $customer['midName'];
