...
$result = db_do("SELECT id FROM users WHERE username='$username'");
	...
	if($dm_id != $user_id) {
		$result = db_do("SELECT id FROM dealers WHERE id='$did'");
	...
	$result = db_do("SELECT d.address1, d.address2, d.bd_account, d.bd_name, d.bd_routing, d.billing_address1, d.billing_address2, 
			...
			d.poc_phone, d.poc_title, d.state, d.topdog_name, d.topdog_title, d.vst_ex, d.vst_no, d.years, d.zip, d.status, d.feed_provider, CONCAT(ae.first_name, ' ', ae.last_name) AS ae_name FROM dealers d, aes ae  WHERE d.id = '$did' AND ae.id = d.ae_id"); 
