...
extract(defineVars("submit", "page_title", "username","arbitrationPol","userPol","privacyPol"));

if ($_SERVER["HTTP_REFERER"] == "http://gdtd.safesecuredemo.com/cioletter.php")
{
	$goBack = false;
	if( $arbitrationPol != 'on')
		//they didn't agree to the arbitration agreement, put them back on that page
		$goBack = true;

	if ($goBack)
		header("Location:http://gdtd.safesecuredemo.com/cioletter.php");
}
else if ($_SERVER["HTTP_REFERER"] == "http://gdtd.safesecuredemo.com/signup/index.php")
{
	//looks like we are coming back from this page, just continue
}
else
{
	//uh, don't think you should be here?
	header("Location:http://gdtd.safesecuredemo.com/cioletter.php");
	exit;
}
