...
class GDTD_Feed_Vinstickers extends GDTD_Feed
{

   protected $feed_name = 'Vinstickers'; 
   const logfile = '/sites/production/feeds/logs/vinstickers_incoming';
   ...
   protected $sql_dealer_count = "SELECT COUNT(*) FROM feed_dealers fd 
      WHERE fd.vinstickers = :dealer_id";
   protected $sql_dealer_info = "SELECT dba, d.id, city, state, zip, payment_method
      FROM dealers d, feed_dealers fd WHERE fd.vinstickers = :dealer_id AND 
      d.id = fd.dealer_id";
   ...
   public function parseData($data)
   {
      ...
      $this->certified = $data[2];
      $this->year = $data[3];
      $this->make = $data[4];
      $this->model = $data[5];
      $this->series = $data[6];
      $this->stock_num = $data[7];
      $this->vin = $data[8];
      $this->miles = $data[9];
      $this->engine_size = $data[11];
      $this->transmission = $data[12];
      $this->ext_color = $data[13];
      $this->int_color = $data[14];
      $this->short_desc = "$data[3] $data[4] $data[5]";
